...
public class OrderServiceTest {
    ...
    public void get_orders_for_customer_and_he_has_orders() {
        Order o1 = new Order();
        String email = "Matt";
        when(orderRepository.findOrdersForCustomer(email)).thenReturn(List.of(order, o1));

        Assertions.assertThat(orderService.getOrdersForCustomer(email)).containsExactly(order, o1);
    }

    @Test
    public void get_orders_for_customer_but_no_orders() {
        String email = "Matt";
        when(orderRepository.findOrdersForCustomer(email)).thenReturn(new ArrayList<>());

        Assertions.assertThat(orderService.getOrdersForCustomer(email)).isEmpty();
    }
